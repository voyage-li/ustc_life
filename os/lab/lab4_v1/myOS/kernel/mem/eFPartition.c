...
typedef struct eFPartition
{
    ...
    unsigned long perSize; // unit: byte（为实际可用空间大小加上一个next空闲块地址的unsigned long类型大小）
...
unsigned long align4(unsigned long size)
{
    unsigned long R = size % 4;
    if (R == 0)
        return size;
    else
        return size + 4 - R;
...
unsigned long eFPartitionInit(unsigned long start, unsigned long perSize, unsigned long n)
{
    ...
    */
    perSize = align4(perSize) + sizeof(unsigned long);
