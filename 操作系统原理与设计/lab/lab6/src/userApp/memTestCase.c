...
int testdP1(int argc, unsigned char **argv)
{
    ...
    if (x)
    {
        ...
        myPrintf(0x7, "MALLOC FAILED, CAN't TEST dPartition\n");
}
...
int testdP2(int argc, unsigned char **argv)
{
    ...
    if (x)
    {
        ...
        if (x1)
            myPrintf(0x5, "success(addr=0x%x)!\n", x1);
        else
            myPrintf(0x5, "failed!\n");
    ...
    if (x)
    {
        ...
        if (x1)
        {
            *(unsigned long *)x1 = 0xAAAAAAAA;
            ...
            myPrintf(0x7, "Alloc memBlock A, failed!\n");
        eFPartitionWalkByAddr(xHandler);
        ...
        if (x2)
        {
            *(unsigned long *)x2 = 0xBBBBBBBB;
            ...
            myPrintf(0x7, "Alloc memBlock B, failed!\n");
        eFPartitionWalkByAddr(xHandler);
        ...
        if (x3)
        {
            *(unsigned long *)x3 = 0xCCCCCCCC;
            ...
            myPrintf(0x7, "Alloc memBlock C, failed!\n");
        eFPartitionWalkByAddr(xHandler);
        ...
        if (x4)
        {
            *(unsigned long *)x4 = 0xDDDDDDDD;
            ...
            myPrintf(0x7, "Alloc memBlock D, failed!\n");
        eFPartitionWalkByAddr(xHandler);
        ...
        if (x5)
        {
            *(unsigned long *)x5 = 0xEEEEEEEE;
            ...
            myPrintf(0x7, "Alloc memBlock E, failed!\n");
        eFPartitionWalkByAddr(xHandler);
...
int testKMEM_max(int argc, unsigned char **argv)
{
    int i = 0x1000;
    unsigned long x;
...
int testKMEM_dpmax(int argc, unsigned char **argv)
{
    unsigned long x, x1, x2, x3, xHandler;
    int i, tsize = 0x100;
    x = kmalloc(tsize);
...
void memTestCaseInit(void)
{
    ...
    addNewCmd("testKMEM_max\0", testKMEM_max, NULL, "test KMEM could max alloc\0");
    addNewCmd("testKMEM_dp\0", testKMEM_dpmax, NULL, "use testdP3 to test kmem\0");
}
