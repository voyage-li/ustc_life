...
def in2pre(s):
    ...
    if i == n and s[0] == LEFT and s[-1] == RIGHT:
        return in2pre(s[1:-1])
    if s[0] == NON and s[1] == LEFT and matchParentheses(
            1) == n and s[-1] == RIGHT:
...
class formula:
    ...
    def __bool__(self):
        return bool(self.preOrderLst)

    def __len__(self):
        return self.sz

    def __getitem(self, i):
        return self.preOrderLst[i]

    def __iter__(self):
        return iter(self.preOrderLst)

    def __repr__(self):
        return 'formula({})'.format(str(self))

    def __str__(self):
        return ''.join([str(i) for i in self.inOrderLst])

    def __hash__(self):
        return hash(''.join([str(i) for i in self.preOrderLst]))

    def __eq__(self, x):
        return isinstance(x, formula) and self.preOrderLst == x.preOrderLst

    def getPreOrderLst(self):
        return self.preOrderLst

    def getInOrderLst(self):
...
class system_L:
    ...
    def prove(self, formulas, x):
        ...
        print('证明: {{{}}} |- {}'.format(', '.join([str(i) for i in formulas]),
                                        x))
        ...
        if pairs:
            ...
            x = pairs[-1].suf
            print('由演绎定理,即证  {{{}}} |- {} '.format(
                ', '.join([str(i) for i in formulas]), x))
        ...
        if p is None:
            ...
            if s is None:
                print(
                    "Sorry! I can't prove this proposition. Maybe it's unprovale "
                )
            else:
                print(s)
                print('证明(1)')
